...
export default {
  data() {
    return {
      inputindex:1,
      // #ifdef H5
      inviteCode: JSON.parse(sessionStorage.getItem("inviteCode")) || null,
      // #endif
      // #ifdef APP-PLUS
  ...
  mounted() {
    ...
    // #ifdef APP-PLUS
    this.sessData = uni.getStorageSync("inviteCode");
    //获取验证码图片
    //获取验证码图片
    this.getDefaultStatus();
...
.register-comp {
  ...
  border-image: linear-gradient(to top, #dcce6a, #c69131);
  border-image-slice: 1;
  border-radius: 8upx;
  padding: 52upx 52upx 62upx;
  ...
  .regBtn {
    ...
    line-height: 72upx;
    border-radius: 40upx;
    background: linear-gradient(90deg, #deb549, #fdea61);
    color: #fff;
    margin: 0 auto;
  }
