    ...
    <view class="header" style="height: 100%">
      ...
      <view class="content">
        ...
        <view class="flexLayout">
          ...
          <text class="recordTextOne">{{ $t("Rp") }}{{ data.amount }}</text>
        </view>
        <view class="flexLayout" v-if="data.type != 10">
          <text class="leftText">{{ $t('提款手续费：') }}</text>
          <text class="recordTextOne">{{ $t("Rp") }}{{ data.administrativeCosts }}</text>
        </view>
        <view class="flexLayout" v-if="data.type != 10">
          <text class="leftText">{{ $t('行政费用：') }}</text>
          <text class="recordTextOne">{{ $t("Rp") }}{{ data.handlingfee }}</text>
        </view>
        <view class="flexLayout" v-if="data.type != 10">
          <text class="leftText">{{ $t('优惠扣除：') }}</text>
          <text class="recordTextOne">{{ $t("Rp") }}{{ data.discountDeduction }}</text>
        </view>
        <view class="flexLayout" v-if="data.type != 10">
          <text class="leftText">{{ $t('到账货币额度：') }}</text>
          <text class="recordTextOne">{{ $t("Rp") }}{{ data.realAmount }}</text>
        </view>
        <view
          class="flexLayout"
          v-if="data.type != 10"
